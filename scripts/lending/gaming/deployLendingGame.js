...
async function main() {
    ...
    const response = await deployer.deploy(ethers.utils.id("LENDING_GAME"), Factory.bytecode, emptyInitializeCalldata)
    await response.wait()
    const lendingAddress = await deployer.get(ethers.utils.id("LENDING_GAME"))
