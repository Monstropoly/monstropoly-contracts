...
async function main() {
    ...
    let emptyInitializeCalldata = await erc721Factory.interface.encodeFunctionData('initialize', []);
    ...
    let [erc721, magicBoxes, upgrader, erc20] = await Promise.all([
    ...
    if (RELAYER_ADDR == ethers.constants.AddressZero) {
        myRelayer = await Relayer.deploy()
    } else {
