...
contract MonstropolyStaking is UUPSUpgradeableByRole {
    ...
    function setFees(uint256 minFee_, uint256 maxFee_)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(
            minFee_ <= maxFee_,
            "MonstropolyStaking: mininum fee must be greater or equal than maximum fee"
        );
        require(
            minFee_ <= 1e20,
            "MonstropolyStaking: minFee cannot exceed 100 ether"
        );
        require(
            maxFee_ <= 1e20,
            "MonstropolyStaking: maxFee cannot exceed 100 ether"
        );
        minFee = minFee_;
        maxFee = maxFee_;
    ...
    function setFeeInterval(uint256 feeInterval_)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function depositFrom(address account, uint256 amount) public whenNotPaused {
        ...
        IMonstropolyERC20 tokenInterface = IMonstropolyERC20(
        ...
        require(
            ...
            "MonstropolyStaking: deposit transfer failed"
        );
    ...
    function withdraw(uint256 amount) public whenNotPaused returns (uint256) {
        ...
        require(
            amount <= user.amount,
            "MonstropolyStaking: user has not enough staking balance"
        );
