...
contract MonstropolyRewardsDistributor {
        ...
        if (block.number > startBlock) {
            if (block.number > endBlock) {
                ...
                if (block.number > changeBlock) {
                    amount +=
                        (increment * ((block.number - changeBlock)**2)) /
                        2;
                }
            }
        }
    ...
    function distribute(address account, uint256 amount) public {
        bytes32 id = IMonstropolyDeployer(
            IMonstropolyDistributionVault(vault).config()
        ).name(msg.sender);
        require(
            amount <= available(id),
            "MonstropolyRewardsDistributor: amount exceeds available"
        );
        distributed[id] += amount;
        IMonstropolyDistributionVault(
            IMonstropolyDeployer(IMonstropolyDistributionVault(vault).config())
                .get(keccak256("DISTRIBUTION_VAULT"))
    ...
    function updateAllocations(uint72[4] memory allocations) public {
        ...
        require(
            total == 100 ether,
            "MonstropolyRewardsDistributor: allocations aggregate must be 100 ether"
        );
    ...
    constructor(
        ...
        uint72[4] memory allocations = [20 ether, 20 ether, 30 ether, 30 ether];
