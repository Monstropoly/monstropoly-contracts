...
contract MonstropolyLendingGame is
    ...
    CoinCharger
{
    ...
    modifier checkBorrower(uint256 tokenId, address borrower) {
        require(
            _checkBorrower(tokenId, borrower),
            "MonstropolyLendingGame: checkBorrower"
        );
        _;
    }
    ...
    function offerLend(
        ...
        uint256 price,
        address payToken
    ) external checkLender(tokenId, _msgSender()) {
        require(
            _lends[tokenId].lender == address(0),
            "MonstropolyLendingGame: token already offered"
        );
    ...
    function cancelLend(uint256 tokenId) external {
        require(
            _msgSender() == _lends[tokenId].lender,
            "MonstropolyLendingGame: only lender can cancel"
        );
        require(
            _lends[tokenId].startDate == 0,
            "MonstropolyLendingGame: lend already taken"
        );
        _unlockToken(tokenId);
        delete _lends[tokenId];
    }

    function takeLend(uint256 tokenId)
        external
        payable
        checkBorrower(tokenId, _msgSender())
        checkLender(tokenId, _lends[tokenId].lender)
    {
    ...
    function claimGamer(uint256 tokenId) external {
        require(
            _checkLendEnd(tokenId),
            "MonstropolyLendingGame: lend not ended"
        );
    ...
    function _checkLender(uint256 tokenId, address lender)
        internal
        view
        returns (bool)
    {
    ...
    function _checkBorrower(uint256 tokenId, address borrower)
        internal
        view
        returns (bool)
    {
        return (_lends[tokenId].borrower == address(0) ||
            _lends[tokenId].borrower == borrower);
    }

    function _checkLendEnd(uint256 tokenId) internal view returns (bool) {
        return
            (_lends[tokenId].startDate + _lends[tokenId].duration) <
            block.timestamp;
    }
